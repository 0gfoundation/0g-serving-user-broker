 ...
 */
function setRpcEndpoint(rpcEndpoint: string, network: 'mainnet' | 'testnet' | 'custom'): void {
 ...
 */
async function promptNetworkSelection(): Promise<{type: 'preset' | 'custom', network?: string, rpc?: string}> {
    ...
    if (networkChoice === 'custom') {
        ...
        try {
            ...
            console.log(chalk.red('Invalid URL format. Please enter a valid URL.'))
            return await promptNetworkSelection()
        }
