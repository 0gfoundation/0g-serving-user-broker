...
export default function network(program: Command) {
    program
        ...
        .action(async () => {
            try {
                ...
                console.log(
                    chalk.gray(
                        'This will configure your default RPC endpoint.\n'
                    )
                )
    ...
    program
        ...
        .action(() => {
            const envRpc =
                process.env['ZG_RPC_ENDPOINT'] || process.env.RPC_ENDPOINT
            ...
            if (envRpc) {
                console.log(
                    chalk.green('✓ RPC Endpoint (Environment):'),
                    chalk.white(envRpc)
                )
            ...
            // Show config file settings
                ...
                console.log(
                    chalk.green('✓ RPC Endpoint (Config):'),
                    chalk.white(config.rpcEndpoint)
                )
                console.log(
                    chalk.green('✓ Network:'),
                    chalk.white(config.network || 'Unknown')
                )
            ...
            console.log(
                chalk.gray(
                    'Config file location: ~/.0g-compute-cli/config.json'
                )
            )
        })
}
